"""Smoke tests for the reusable trainer engine."""
...
class EngineSmokeTests(unittest.TestCase):
    ...
    def test_keyed_batch_adapter_runs(self):
        ...
        metrics = evaluate(model         = nn.Linear(4, 1),
                           dataloader    = loader,
                           loss_fn       = nn.MSELoss(),
                           device        = "cpu",
                           batch_adapter = KeyedBatchAdapter("image", "labels"),
                          )
    ...
    def test_load_training_config_accepts_amp_dtype(self):
        config = load_training_config({"device"    : "cpu",
                                       "use_amp"   : False,
                                       "amp_dtype" : "bf16"})
